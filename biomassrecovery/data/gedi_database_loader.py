...
class FileTracker(object):
    def __init__(self, dir: Path, checkpoint_file: Path):
        ...
        completed = set(
            Path(line.strip()) for line in open(checkpoint_file, "r")
        )
...
def parse_file(file: Path) -> gpd.GeoDataFrame:
    ...
    for beam in tqdm(granule.iter_beams(), total=granule.n_beams):
        try:
            ...
            logger.warn(
                f"Data for beam {beam.name} in granule {file.name} incomplete"
            )
...
def bulk_import_from_directory(
    ...
    table = db.Table(
        table_name, db.MetaData(), autoload=True, autoload_with=engine
    )
