...
def compute_recovery_period(
    ...
    if include_nonforest:
        ...
        recovery_land_type = (last_deforested.data >= last_nonforest.data).astype(np.float32)
        
        last_deforested.data = np.maximum(
            last_deforested.data, last_nonforest.data
        )
        # create array with same coordintate labels and recovery_land_type as data
    ...
    if as_startyear:
        return last_deforested.where(recovery), recovery_land_type_DataArray.where(recovery)
    
    # years since last deforested or nan if not recovering
    recovery_period = survey_year - last_deforested.where(recovery)
    return recovery_period, recovery_land_type_DataArray.where(recovery)
